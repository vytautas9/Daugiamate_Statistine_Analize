...
ro <- R[2,1] / ( sqrt(R[1,1]) * sqrt(R[2,2]) )
...
DvimaciaiGauso <- list()
matrica <- R
koordinate <- seq(40, by = 10, length.out = 8)
for(i in 1:8){
   matrica[1,1] <- koordinate[i]
   DvimaciaiGauso[[i]] <- DvimatisGausoAD(1000, M, matrica)
   print(matrica[2,1] / ( sqrt(matrica[1,1]) * sqrt(matrica[2,2]) ))
   print(eigen(matrica)$values >= 0)
   
}
par(mfrow=c(4,2))
for(i in 1:8){
  item <- paste("N - 1000, 1,1 koordinates reiksme - ",koordinate[i],sep="")
  plot(DvimaciaiGauso[[i]],xlab="X1",ylab="X2",main=item, col=i)
  ellipses(DvimaciaiGauso[[i]],.5)
  ellipses(DvimaciaiGauso[[i]],.05)
  abline(v = apply(DvimaciaiGauso[[i]],2,mean)[1], col="red")
...
DvimaciaiGauso <- list()
matrica <- R
koordinate <- seq(35, 27, by = -2)
for(i in 1:length(koordinate)){
   matrica[1,1] <- koordinate[i]
   print(matrica[2,1] / ( sqrt(matrica[1,1]) * sqrt(matrica[2,2]) ))
   print(eigen(matrica)$values >= 0)
   DvimaciaiGauso[[i]] <- DvimatisGausoAD(1000, M, matrica)
